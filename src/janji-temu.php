...
// Ambil data divisi
$divisi = fetchAll(query("SELECT id, nama_divisi FROM ref_divisi WHERE deleted_at IS NULL ORDER BY id"));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_divisi)) $errors['id_divisi'] = 'Pilih staf/divisi tujuan';
    if (empty($tanggal_waktu)) $errors['tanggal_waktu'] = 'Pilih tanggal dan waktu';

    if (empty($errors)) {
        $sql = "INSERT INTO janji_temu (nama, email, id_divisi, tanggal_waktu, topik_diskusi) VALUES ($1, $2, $3, $4, $5)";
        $result = pg_query_params($db, $sql, [$nama, $email, $id_divisi, $tanggal_waktu, $topik_diskusi ?: null]);
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                <div class="p-6 md:p-8">
                    ...
                    <form method="POST" class="space-y-5">
                        ...
                        <!-- Ingin Bertemu Siapa -->
                        <div>
                            <label for="id_divisi" class="block text-sm font-medium text-slate-700 mb-2">Ingin Bertemu Siapa?</label>
                            <select 
                                id="id_divisi" 
                                name="id_divisi"
                                class="w-full px-4 py-3 border <?= isset($errors['id_divisi']) ? 'border-red-300' : 'border-slate-200' ?> rounded-lg focus:ring-2 focus:ring-sky-400 focus:border-sky-400 outline-none text-sm bg-white"
                            >
                                <option value="">- Pilih Staf / Divisi -</option>
                                <?php foreach ($divisi as $d): ?>
                                    <option value="<?= $d['id'] ?>" <?= ($_POST['id_divisi'] ?? '') == $d['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($d['nama_divisi']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <?php if (isset($errors['id_divisi'])): ?>
                                <p class="mt-1 text-xs text-red-500"><?= $errors['id_divisi'] ?></p>
                            <?php endif; ?>
                        </div>
